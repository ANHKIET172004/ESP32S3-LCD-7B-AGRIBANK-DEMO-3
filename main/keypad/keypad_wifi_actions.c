...
void hide_wifi_input() {
    ...
    if (g_keypad.wifi_step != 1) return;   // chỉ áp dụng cho password
    ...
    // ---------------------------
    // TẠO VIEW 16 KÝ TỰ GIỐNG HỆ NHẬP SSID
    // ---------------------------
    char view[17];
    int len = g_keypad.ssid_len;
...
void update_wifi_input_buffer(char key) {
    ...
    // --- INSERT ---
        ...
        //if (g_keypad.ssid_len < WIFI_MAX_LEN) {
        if (g_keypad.ssid_len < WIFI_MAX_LEN) {
    ...
    // buffer_index = ssid_len (LUÔN LUÔN)
    g_keypad.buffer_index = g_keypad.ssid_len;
...
void delete_wifi_input_key() {
    ...
    // Nếu chuỗi ngắn hơn LCD → reset
    if (g_keypad.ssid_len <= 16) {
        g_keypad.ssid_window_start = 0;
    }
    else {
        // Nếu cursor lùi về đầu cửa sổ → kéo window về trái
        if (g_keypad.ssid_real_pos < g_keypad.ssid_window_start) {
            g_keypad.ssid_window_start = g_keypad.ssid_real_pos;
        }

        // Nếu window vượt quá phần cuối buffer → kéo ngược lại
        int max_window_start = g_keypad.ssid_len - 16;
...
void enter_wifi() {
    ...
    // =============================
    // BƯỚC 1: XÁC NHẬN SSID
    // =============================
    if ((g_keypad.wifi_step == 0) && (g_keypad.ssid_len > 0)) {
        ...
        g_keypad.wifi_step = 1;
        g_keypad.hide = true;  // password mặc định hide

        // Hiển thị password input
        lcd_show_wifi_pass("");
        lcd_put_cur(1, 0);
        lcd_send_cmd(0x0F);
    ...
    // =============================
    // BƯỚC 2: XÁC NHẬN PASSWORD
    // =============================
    else if ((g_keypad.wifi_step == 1) && (g_keypad.ssid_len > 0)) {
        ...
        // Restore input buffer sau khi nhập xong
        ...
        g_keypad.ssid_len = g_keypad.saved_buffer_index;
        g_keypad.ssid_real_pos = g_keypad.ssid_len;
        if (g_keypad.ssid_len > 16)
            g_keypad.ssid_window_start = g_keypad.ssid_len - 16;
        else
            g_keypad.ssid_window_start = 0;
